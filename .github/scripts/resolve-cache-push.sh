...
# `.#cache-push`, which re-exports the kasha flake input and is therefore
...
ref='github:Zebradil/kasha#cache-push'
if nix eval --raw '.#cache-push.outPath' >/dev/null 2>&1; then
  ref='.#cache-push'
fi
