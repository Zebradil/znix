...
One file per workaround, at `modules/flake/workarounds/<package>.nix`. It holds
...
```bash
git add modules/flake/workarounds/<package>.nix
...
overlaid derivation against the vanilla one — these two should differ on an
affected system and match on an unaffected one:
...
dispatch). For each workaround it builds `#workaroundProbes.<system>.<name>`
— the package straight from `nixpkgs`, no overlays — on every system the
workaround claims. A claimed system with no CI runner is warned about and
...
- Workaround names map to nixpkgs attributes, so one package cannot be pinned to
  different revisions on different systems.
